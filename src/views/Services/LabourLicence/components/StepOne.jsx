...
const StepOne = () => {
  ...
  const fetchTaskIds = async () => {
    ...
    if (res.status_cd === 0 && res.data?.task_data) {
      console.log('Task Data:', res.data.task_data);
      setTaskIds({
      ...
      console.log('Task IDs:', {
        businessIdentityTaskId: res.data.task_data["Business Identity Structure"]?.task_id,
        promoterSignatoryTaskId: res.data.task_data["Signatory Details"]?.task_id,
        businessPremisesTaskId: res.data.task_data["Business Location Proofs"]?.task_id
      });
    }
  };
