...
const AddUser = ({ open, onClose, user, getUsers }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        let __postData = {
          context_id: user.active_context.id,
        ...
        let response = await Factory('post', '/user_management/team/member/add/', __postData, {});
        if (response.res.status_cd === 0) {
          setSnackbar({
            open: true,
            message: 'User added successfully! An email has been sent with login details.',
            severity: 'success'
          });
          resetForm();
          onClose();
          ...
          setSnackbar({
            open: true,
            message: response.res.message || 'Failed to add user. Please try again.',
            severity: 'error'
          });
        }
      } catch (error) {
          ...
          <DialogContent dividers>
            ...
            >
              ...
              >
                ...
                By clicking "Add User", an email will be sent with their login details account activation link. The role selected will
                automatically give them basic access, which you can change under "Modify Permissions".
              </Typography>
            </Box>
            <Box sx={{ mb: 3 }}>
              <ToggleButtonGroup
