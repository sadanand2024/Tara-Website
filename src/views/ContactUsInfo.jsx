...
const ContactUsInfo = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await Factory('get', '/user_management/list-contacts/');
  ...
  const filtered = contacts.filter(
    (info) =>
      (info.first_name && info.first_name.toLowerCase().includes(search.toLowerCase())) ||
      (info.last_name && info.last_name.toLowerCase().includes(search.toLowerCase())) ||
        ...
        <Grid2 container spacing={3}>
          {paginated.map((info, idx) => (
            <Grid2 size={{ xs: 12, sm: 12, md: 6, lg: 3, xl: 3 }} key={info.id || info.email + idx}>
              <Card
                sx={{
                  height: { xs: '398px', sm: '398px', md: '598px', lg: '498px', xl: '398px' },
                  border: '1px solid #e3e3e3',
                  ...
                  <Stack spacing={2} sx={{ flexGrow: 1 }}>
                    ...
                    <Stack direction="row" spacing={2}>
                      ...
                      <Typography variant="body2">
                        <b>Date & Time</b>
                        <br />
                        {new Date(`${info.created_date} ${info.created_time}`).toLocaleString()}
                      </Typography>
                    </Stack>
                  </Stack>
